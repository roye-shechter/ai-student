...
export function MarkdownMessage({ content }: { content: string }) {
  return (
    <div className="space-y-2 text-sm leading-relaxed [&_strong]:text-[#ffb066] [&_a]:text-[#ffb066] [&_a]:underline">
      <Markdown
        remarkPlugins={[remarkGfm, remarkMath]}
        rehypePlugins={[rehypeKatex]}
        components={{
          h1: ({ children }) => <h1 className="font-serif text-base text-[#ffb066] mt-2 mb-1">{children}</h1>,
          h2: ({ children }) => <h2 className="font-serif text-base text-[#ffb066] mt-2 mb-1">{children}</h2>,
          h3: ({ children }) => <h3 className="font-serif text-sm text-[#ffb066] mt-2 mb-1">{children}</h3>,
          ...
          code: ({ children }) => (
            <code className="bg-[#0a0e14] border border-[#242b3a] rounded-sm px-1 py-0.5 text-[#ffb066] text-xs">
              {children}
            </code>
          ),
          blockquote: ({ children }) => (
